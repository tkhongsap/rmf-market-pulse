/**
 * Test SCBGOLDHRMF fund using SEC Fund Daily Info API
 *
 * This script tests fetching SCBGOLDHRMF (SCB Gold THB Hedged RMF) data
 * from the official Thailand SEC API
 *
 * Fund Details:
 * - Abbreviation: SCBGOLDHRMF
...
async function testSCBGOLDHRMF() {
  console.log(colors.bright + colors.yellow);
  console.log('╔════════════════════════════════════════════════════════════════════════════╗');
  console.log('║          SCBGOLDHRMF Fund Data - SEC Fund Daily Info API                  ║');
  console.log('║              SCB GOLD THB HEDGED RMF (M0778_2554)                          ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════╝');
  ...
  try {
    ...
    if (!apiKey) {
      ...
      console.log(`${colors.yellow}Get your key from: https://api-portal.sec.or.th/${colors.reset}`);
      return;
    }
    ...
    printSection('📊 Test 2: Fetch Latest NAV for SCBGOLDHRMF');

    const today = new Date();
    const fundId = 'M0778_2554'; // SCBGOLDHRMF project ID
    ...
    console.log(`  Fund Name:          SCBGOLDHRMF (SCB Gold THB Hedged RMF)`);
    ...
    console.log(`  ✓ SCBGOLDHRMF data is accessible`);
    console.log(`  ✓ Latest NAV: ${formatNumber(latestNav.last_val)} THB (${latestNav.nav_date})`);
...
// Run the test
console.log(colors.blue + 'Starting SCBGOLDHRMF test using SEC Fund Daily Info API...\n' + colors.reset);
testSCBGOLDHRMF();
